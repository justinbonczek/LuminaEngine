...
void Camera::Initialize(Lumina::Window& window)
{
	SetPosition(0.0f, 0.0f, -10.0f);
	SetLens(0.25f * 3.1415926535f, window.AspectRatio(), 0.1f, 200.0f);
}
...
XMFLOAT4X4 Camera::ViewTranspose()const
{
	XMFLOAT4X4 temp;
	XMStoreFloat4x4(&temp, XMMatrixTranspose(XMLoadFloat4x4(&m_View)));
	return temp;
}

XMMATRIX Camera::ProjXM()const
{
...
XMFLOAT4X4 Camera::ProjTranspose() const
{
	XMFLOAT4X4 temp;
	XMStoreFloat4x4(&temp, XMMatrixTranspose(XMLoadFloat4x4(&m_Proj)));
	return temp;
}
NS_END